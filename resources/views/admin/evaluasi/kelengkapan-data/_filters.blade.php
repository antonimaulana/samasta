<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
    <div>
        <label for="fresh_days" class="mb-1 block text-sm font-medium text-gray-700">Ambang Mutakhir</label>
    ...
    <div>
        <label for="status_data" class="mb-1 block text-sm font-medium text-gray-700">Kelengkapan</label>
        <select name="status_data" id="status_data"
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
            <option value="">Semua Status</option>
            @foreach ($daftarStatusData as $value => $label)
                <option value="{{ $value }}" @selected($statusFilter === $value)>{{ $label }}</option>
            @endforeach
        </select>
    </div>
    <div>
        <label for="mutakhir" class="mb-1 block text-sm font-medium text-gray-700">Kemutakhiran</label>
        <select name="mutakhir" id="mutakhir"
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
            <option value="">Semua</option>
            @foreach ($daftarMutakhir as $value => $label)
                <option value="{{ $value }}" @selected($mutakhirFilter === $value)>{{ $label }}</option>
            @endforeach
        </select>
    </div>
</div>
