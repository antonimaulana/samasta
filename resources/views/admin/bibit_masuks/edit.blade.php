...
@section('header', 'Edit Stok Masuk')
    ...
    <div class="max-w-3xl rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div class="mb-6 rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
            ...
            <p class="mt-1 font-medium text-gray-900">
                <x-admin.bibit-nama :bibit="$masuk->bibit" /> ({{ $masuk->bibit->jenis }})
            </p>
            @if ($masuk->jumlah > $masuk->sisa_stok)
                <p class="mt-1 text-xs text-amber-700">
                    {{ $masuk->jumlah - $masuk->sisa_stok }} bibit dari batch ini sudah dikeluarkan.
                    Jumlah minimum: {{ $masuk->jumlah - $masuk->sisa_stok }}.
                </p>
            @endif
        </div>

        <form action="{{ route('admin.bibit-masuks.update', $masuk) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    ...
                    <input type="number" name="jumlah" id="jumlah"
                           min="{{ max(1, $masuk->jumlah - $masuk->sisa_stok) }}"
                           value="{{ old('jumlah', $masuk->jumlah) }}"
                           required
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_masuk" id="tanggal_masuk"
                           value="{{ old('tanggal_masuk', $masuk->tanggal_masuk->format('Y-m-d')) }}"
                           required
                           class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-green-500 focus:outline-none focus:ring-1 focus:ring-green-500">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="sumber" id="sumber" required
                    ...
                    @foreach (\App\Models\Bibit::SUMBER as $sumber)
                        <option value="{{ $sumber }}" @selected(old('sumber', $masuk->sumber) === $sumber)>{{ $sumber }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                <label class="flex items-center gap-3 rounded-lg border border-green-100 bg-green-50 px-4 py-3">
                    <input type="checkbox" name="status_siap_tanam" value="1"
                           @checked(old('status_siap_tanam', $masuk->status_siap_tanam))
            ...
            <div>
                ...
                @if ($masuk->foto_url)
                    <a href="{{ $masuk->foto_url }}" target="_blank" rel="noopener" class="mb-2 block">
                        <img src="{{ $masuk->foto_url }}" alt="Foto saat ini" class="max-h-40 rounded-lg border object-cover">
                    </a>
                @endif
